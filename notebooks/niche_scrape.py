...
def parse_niche(page):
    ...
    # Name
    place["Name"] = (
        page.xpath("//h1[@class='postcard__title']")[0]
        .text_content()
        .replace(" Township", "")
        .replace(" Boro", "")
    )
    place["State"] = (
        page.xpath("//ul[@class='profile-breadcrumbs']")[0]
        .getchildren()[0]
        .text_content()
    )
    place["County"] = (
    ...
    # Scores
    report_card = (
        page.get_element_by_id("report-card")
        .getchildren()[1]
        .getchildren()[0]
        .getchildren()[0]
    )
    ...
    overall_score_card = (
        report_card.getchildren()[0]
        .xpath("//div[@class='overall-grade__niche-grade']")[0]
        .getchildren()
    )
    individual_score_card = (
        report_card.getchildren()[1]
        .xpath("//ol[@class='ordered__list__bucket']")[0]
        .getchildren()
    )
    ...
    for score_name in individual_scores:
        individual_scores[score_name] = individual_scores[score_name].replace(
            " minus", "-"
        )
        individual_scores[score_name] = individual_scores[score_name].replace(
            " plus", "+"

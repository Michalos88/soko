    ...
    for req in weekend_commute:
        ...
        for addr_idx in range(0, len(cur_addresses)):
            duration_text.append(
                req["rows"][0]["elements"][addr_idx]["duration"]["text"]
            )
            duration_value.append(
                req["rows"][0]["elements"][addr_idx]["duration"]["value"]
            )

            duration_in_traffic_text.append(
                req["rows"][0]["elements"][addr_idx]["duration_in_traffic"]["text"]
            )
            duration_in_traffic_value.append(
                req["rows"][0]["elements"][addr_idx]["duration_in_traffic"]["value"]
            )

            distance_text.append(
                req["rows"][0]["elements"][addr_idx]["distance"]["text"]
            )
            distance_value.append(
                req["rows"][0]["elements"][addr_idx]["distance"]["value"]
            )
